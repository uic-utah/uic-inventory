...
import { createPdfDocument, generatePdfDefinition } from "./pdfHelpers.js";
import { getPrintMapImageAsync } from "./printService.js";

http("generate", async (req, res) => {
  const { contacts, inventory } = req.body;
  ...
  const wells = inventory.wells.map((well) => {
    return {
  ...
  const image = await getPrintMapImageAsync(site, wells);

  const definition = generatePdfDefinition(inventory, contacts, image, true);

  var pdf = await createPdfDocument(definition);
