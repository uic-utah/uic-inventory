...
export const getPdfAssets = async (bucket, metadata, seal) => {
  ...
  try {
    ...
    console.error("could not get binary images");
  }
  try {
    pdfs = await getBinaryPdfs(bucket, pdfPaths);
  } catch (error) {
    console.error("could not get binary pdfs");
  }
...
const getBase64Images = async (bucket, metadata) => {
  ...
  Object.entries(metadata).forEach(([key, fileName]) => {
    console.debug("creating stream for", key, {
      structuredData: true,
    });
...
const getPdfData = (stream) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on("error", (err) => {
      console.error("getPdfData error", err, { structuredData: true });
...
const addExtraImages = (images) => {
  const extras = Object.keys(images ?? {}).filter((key) => key.startsWith("extra"));
...
const addSiteContacts = (data) => {
  console.log("adding site contacts");
...
const addLocationDetails = () => {
  console.log("adding location details");
...
const addWellInformation = (data) => {
  console.log("adding well information");
...
const addWaterSystemInformation = (data) => {
  console.log("adding water system information");

  const columnDefinitions = buildSubTableDefinitionFor(data);

  if (columnDefinitions.length === 0) {
    console.log("no water system information found");
...
const addInjectateCharacterization = (data) => {
  console.log("adding injectate characterization");

  const columnDefinitions = buildColumnDefinitionFor(data);

  if (columnDefinitions.table.body.length === 0) {
    console.log("no injectate characterization found");
...
const addHydrogeologicCharacterization = (data) => {
  console.log("adding hydrogeologic characterization");

  const columnDefinitions = buildColumnDefinitionFor(data);

  if (columnDefinitions.table.body.length === 0) {
    console.log("no hydrogeologic characterization found");
...
const addAppendixItems = () => {
  console.log("adding appendix items");
...
export const createPdfDocument = async (definition, extraPdfPages) => {
  ...
  // console.debug("saved front page now appending extra pages", Object.keys(extraPdfPages).length, {
  //   structuredData: true,
  // });
...
export const appendPdfPages = async (original, metadata) => {
  ...
  for (let i = 0; i < pdfs.length; i++) {
    ...
    const copiedPages = await source.copyPages(extraPages, extraPages.getPageIndices());
