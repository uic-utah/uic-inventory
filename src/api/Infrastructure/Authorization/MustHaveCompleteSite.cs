          ...
          CancellationToken token = default) {
            if (_metadata.Site.Status != SiteStatus.Complete) {
                _log.ForContext("site", _metadata.Site)
                    .ForContext("authorization", "MustHaveCompleteSite:S01")
                    .Warning("Cannot add well to incomplete site");

                return Task.FromResult(AuthorizationResult.Fail("S01:You must complete your site information before submitting a well."));
            }
